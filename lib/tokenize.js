    ...
    switch (code) {
      ...
      case asterisk:
        ...
        let prevChar = css.slice(pos - 1, pos),
          rStrict = /[a-z0-9\.]/gi,
          rLoose = /[a-z\.]/gi,
          rOper = options.loose ? rLoose : rStrict,
          lastToken = tokens.slice(-1)[0],
          lastPrevToken = tokens.length > 1 ? tokens.slice(-2)[0] : [],
          isOperator = false;
        ...
        if (!options.loose && pos > 0 && (code === plus || code === minus)) {
        ...
        isOperator = !rOper.test(nextChar);

        if (options.loose && isOperator) {
          if (lastToken && lastToken[0]) {
            if (lastToken[0] === 'space') {
              if (lastPrevToken[0] && lastPrevToken[0] === 'operator') {
                isOperator = false;
              }
              else {
                isOperator = false;
            ...
            else if (lastToken[0] === 'operator') {
              isOperator = false;
            }
          }
          else if (nextChar.charCodeAt(0) !== plus && nextChar.charCodeAt(0) !== minus) {
            isOperator = false;
          }
        }
